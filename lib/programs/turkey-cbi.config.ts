...
export const turkeyCbiConfig: ProgramConfig = {
  ...
  footnotes: [
    ...
    "Real estate route only: the property valuation report and the title deed processing tax are charged per property and scale with the number of properties selected. Title deed tax (4%) and the agent fee (2%) are calculated on the property value on file with GCS — known limitation, flagged internally: the current calculation uses a single illustrative property value regardless of how many properties are selected; confirm with GCS whether this should scale before quoting a 2-property Real Estate case.",
  ],
};
